...
`PolicySnapshot` in core (no JSON library). Formats implement `PolicyCodec`.
...
A document **must** include `validity`. Presets already have a default rule, and that rule
is written out (`"orDefault": "P90D"` for HTTPS). Hand-written JSON that omits `validity`
is rejected; the library does not guess 90 days.
